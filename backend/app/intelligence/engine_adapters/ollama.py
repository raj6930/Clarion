...
class OllamaEngine(BaseEngine):
    ...
    async def generate(
        ...
        try:
            ...
            raise EngineError(f"Ollama HTTP error: {e.response.status_code}", self.engine_name, retryable=e.response.status_code >= 500)
        except httpx.ConnectError:
            raise EngineError("Cannot connect to Ollama. Is the container running?", self.engine_name, retryable=True)
        ...
        return EngineResponse(
            ...
            usage=usage, latency_ms=latency, cost_usd=0.0, raw_response=data,
        )

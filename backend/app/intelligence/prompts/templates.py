...
def review_draft_prompt(case_data: dict, events: list[dict], rubric: list[dict]) -> tuple[str, str]:
    ...
    rubric_text = "\n".join(f"- [{q.get('id')}] {q.get('text')} (weight: {q.get('weight', 1)})" for q in rubric)
    events_text = "\n".join(
        f"[{e.get('timestamp')}] {e.get('event_type')}: {e.get('body', '')[:300]}" for e in events[:30]
    )
    user = (
...
def anon_context_review_prompt(text: str, detected_entities: list[dict]) -> tuple[str, str]:
    ...
    user = f"Already detected:\n{already}\n\nText to review:\n{text}\n\nList any additional PII not yet detected."
    return system, user

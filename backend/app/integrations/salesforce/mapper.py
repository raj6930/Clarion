...
def map_priority(sf_priority: str | None) -> str:
...
def parse_email_addresses(sf_addresses: str | None) -> list[str]:
...
def extract_thread_id(subject: str | None) -> str | None:
...
def clean_subject(subject: str | None) -> str | None:
    ...
    # Remove RE: FW: prefixes for cleaner display (loop for nested prefixes)
    prev = None
    while prev != cleaned:
        prev = cleaned

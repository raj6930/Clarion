 ...
 * Hero metric with inline SVG sparkline trend line.
 * Phase 0: Stub with placeholder. Phase 7: Live data rendering.
 */
import { type WidgetProps } from './registry';

export default function SparklineCard({ metric, size = 'md' }: WidgetProps) {
  return (
    <div className="bg-[var(--clarion-surface)] rounded-[var(--clarion-radius)] border border-[var(--clarion-border)] p-[var(--clarion-card-padding)] h-full">
      ...
      <p className="text-xs text-[var(--clarion-text-muted)]">SparklineCard — {size}</p>
    </div>
  );
}
